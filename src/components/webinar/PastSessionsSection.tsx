        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          ...
          <Card className="group overflow-hidden bg-card border-border/50 hover:shadow-hover transition-all duration-300 cursor-pointer">
            <div className="relative aspect-video overflow-hidden">
              <img src={featuredVideo.thumbnail} alt={featuredVideo.title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
          ...
          <div className="flex flex-col gap-6">
            {highlights.map((highlight, index) => <Card key={index} className="group overflow-hidden bg-card border-border/50 hover:shadow-hover transition-all duration-300 cursor-pointer flex-1">
                <div className="flex flex-col sm:flex-row h-full">
                  <div className="relative sm:w-2/5 aspect-video sm:aspect-auto overflow-hidden">
                    <img src={highlight.thumbnail} alt={highlight.title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                    <div className="absolute inset-0 bg-gradient-to-br from-brand-dark/80 to-brand-medium/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <div className="w-12 h-12 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center">
                        <Play className="w-6 h-6 text-white fill-white" />
                      </div>
                    </div>
                    <Badge className={`absolute top-3 left-3 text-xs ${highlight.category === "Lecture Highlight" ? "bg-primary text-primary-foreground" : "bg-accent text-accent-foreground"}`}>
                      {highlight.category}
                    </Badge>
                  </div>
