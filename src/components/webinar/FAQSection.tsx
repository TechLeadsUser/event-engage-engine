      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <Accordion type="single" collapsible className="space-y-4 mb-12">
          {faqs.map((faq, index) => (
            <AccordionItem 
              key={index} 
              value={`item-${index}`}
              className="bg-card border border-border/50 rounded-lg px-6 data-[state=open]:border-l-4 data-[state=open]:border-l-accent shadow-sm hover:shadow-card transition-all duration-300"
            >
        ...
        <Card className="bg-gradient-card border-border/50 p-8 text-center shadow-card">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <button className="px-6 py-3 border-2 border-primary text-primary hover:bg-primary hover:text-primary-foreground rounded-lg font-medium transition-all duration-300 flex items-center justify-center gap-2">
